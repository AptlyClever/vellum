...
def test_propose_intake_has_honest_steps(tmp_path: Path, monkeypatch, pg_intake) -> None:
...
def test_unity_plan_includes_reconcile(tmp_path: Path, monkeypatch, pg_intake) -> None:
    reg = tmp_path / "register.yaml"
    ensure_register(force_reseed=True)
...
def test_patch_step_rollups(tmp_path: Path, monkeypatch, pg_intake) -> None:
    reg = tmp_path / "register.yaml"
    ensure_register(force_reseed=True)
...
def test_intake_api(tmp_path: Path, monkeypatch, pg_intake) -> None:
    reg = tmp_path / "register.yaml"
    monkeypatch.delenv("VELLUM_VAULT_REGISTER_PATH", raising=False)
