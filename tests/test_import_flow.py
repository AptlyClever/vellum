...
def test_import_status_and_mark(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("VELLUM_SEED_PATH", str(Path(__file__).resolve().parents[1] / "config" / "seed-catalog.yaml"))
...
def test_stage_upload_extracts(tmp_path: Path, monkeypatch) -> None:
    ...
    with zpath.open("rb") as fh:
        res = client.post(
            ...
            data={
                "host_content_path": r"D:\Games\VellumLibrary\Content\HangarX",
...
def test_content_folders_and_host_stage_enqueue(tmp_path: Path, monkeypatch) -> None:
    ...
    register_mod.ensure_register(force_reseed=True)
    hangar_path = r"D:\Games\VellumLibrary\Content\HangarX"
    (specs / "factory.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "host_id": "factory",
                    ...
                    "content_folders": [
                        {
                            "name": "HangarX",
                            "path": hangar_path,
                            "project_root": r"D:\Games\VellumLibrary",
                            "engine": "unreal",
                        }
                    ],
                    "content_root_path": r"D:\Games\VellumLibrary\Content",
                    "fab_target_project": r"D:\Games\VellumLibrary\VellumLibrary.uproject",
                    "fab_target_label": "VellumLibrary (F:)",
                },
            }
        ),
        encoding="utf-8",
    ...
    client = TestClient(app)
    folders = client.get("/api/ue/hosts/content-folders?host_id=factory")
    assert folders.status_code == 200
    assert folders.json()["count"] == 1
    assert folders.json()["folders"][0]["name"] == "HangarX"
    assert "fab_target_project" in folders.json()

    refresh = client.post("/api/ue/hosts/content-folders/refresh?host_id=factory")
    ...
    bad = client.post(
        ...
        json={
            "host_content_path": r"C:\dev\VellumLibrary\Content\HangarX",
            "content_folder_name": "HangarX",
            "ue_host": "factory",
        },
    )
    ...
    listed = ue_hosts_mod.list_content_folders("factory")
...
def test_fab_install_enqueue(tmp_path: Path, monkeypatch) -> None:
    ...
    # Isolate from live Factory Content scan (hangar already on F: would skip VaultCache).
    monkeypatch.setattr(
        ue_hosts_mod,
        "list_content_folders",
        lambda host_id=None: {"schema_version": 1, "folders": [], "count": 0},
    ...
    ok = client.post(
        ...
        json={"ue_host": "factory", "auto_stage": True},
    )
...
def test_register_orphans(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("VELLUM_JOBS_DB_PATH", str(tmp_path / "jobs.sqlite"))
    register_mod.ensure_register(force_reseed=True)
    free_path = r"D:\Games\VellumLibrary\Content\CookiePack"
    (specs / "factory.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
    ...
    res = client.post(
        ...
        json={"folders": ["CookiePack"], "auto_stage": True, "ue_host": "factory"},
    )
    assert res.status_code == 200, res.text
    body = res.json()
...
def test_resolve_folder_fuzzy_maps_marble(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv(
        "VELLUM_SEED_PATH",
        str(Path(__file__).resolve().parents[1] / "config" / "seed-catalog.yaml"),
    )
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
    register_mod.ensure_register(force_reseed=True)
...
def test_game_ready_elements_count_as_conversion_evidence(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    monkeypatch.setenv(
        "VELLUM_SEED_PATH",
        str(Path(__file__).resolve().parents[1] / "config" / "seed-catalog.yaml"),
    )
