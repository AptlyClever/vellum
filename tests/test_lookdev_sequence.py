...
def test_ingest_sequence_zip(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
...
def test_ingest_sequence_multi_lane_one_write(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    vault = _seed(tmp_path, monkeypatch)
    from backend.main import app
