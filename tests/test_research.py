...
def _seed(tmp_path: Path, monkeypatch, pg_catalogs: str, *, token: str = WRITE_TOKEN) -> Path:
...
def test_ingest_all_supported_formats(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    vault = _seed(tmp_path, monkeypatch, pg_catalogs)
    samples = [
...
def test_ingest_video_via_api_and_list(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    """WebM upload through POST /api/visual-research appears in list."""
    _seed(tmp_path, monkeypatch, pg_catalogs)
...
def test_search_and_distinguish_from_game_assets(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    _seed(tmp_path, monkeypatch, pg_catalogs)
...
def test_catalog_persists_and_vault_mirror(
    tmp_path: Path, monkeypatch, pg_catalogs
) -> None:
    """Superseded name: this used to prove a cache-clear didn't lose the row.

    Storage is Postgres now (PX001), not a cached YAML read -- there is no cache
    to clear. What still matters is the same as before: the item is durably
    there on a fresh read, and the vault mirror is a faithful export of it.
    """
    vault = _seed(tmp_path, monkeypatch, pg_catalogs)
    item = research_mod.ingest_image(
        data=_png_bytes(),
...
def test_rejects_invalid_and_unsafe_content(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    _seed(tmp_path, monkeypatch, pg_catalogs)
    try:
...
def test_path_jail(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    _seed(tmp_path, monkeypatch, pg_catalogs)
...
def test_missing_write_token_blocks_mutations(tmp_path: Path, monkeypatch, pg_catalogs) -> None:
    _seed(tmp_path, monkeypatch, pg_catalogs, token="")
    from backend.main import app
