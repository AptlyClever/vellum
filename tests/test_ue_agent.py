...
def test_ue_hosts_factory_is_active() -> None:
    payload = ue_hosts_mod.public_hosts_payload()
    assert payload["active"] == "factory"
    assert payload["active_host"]["id"] == "factory"
    assert "D:\\Games\\UE_5.8" in payload["active_host"]["ue_editor"]
    ids = {h["id"] for h in payload["hosts"]}
    assert ids == {"factory", "studio"}
...
def test_ue_host_specs_roundtrip(tmp_path: Path, monkeypatch) -> None:
    ...
    assert posted.json()["host_id"] == "factory"
...
def test_ue_capture_claim_is_not_taken_by_linux_kinds(tmp_path: Path, monkeypatch) -> None:
    ...
    hosts = client.get("/api/ue/hosts")
    assert hosts.status_code == 200
    assert hosts.json()["active"] == "factory"
    ...
    reported = client.post(
        ...
        json={
            "result": {"notes": "test", "ue_host": "factory"},
            "scratch_project_path": r"D:\Games\VellumLibrary",
            "engine_version": "5.8",
        },
    )
