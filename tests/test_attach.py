...
def _seed_output(tmp_path: Path, monkeypatch, pg_catalogs: str, *, lane: str = "hail-overlay", kind: str = "niagara-render") -> dict:
    ...
    seed_catalog(pg_catalogs, "derived_outputs", {"schema_version": 1, "outputs": [row]})
    return row


def test_attach_lcard_and_bandit(tmp_path, monkeypatch, pg_catalogs):
    _seed_output(tmp_path, monkeypatch, pg_catalogs)
...
def test_attach_hail_copies_without_register(tmp_path, monkeypatch, pg_catalogs):
    _seed_output(tmp_path, monkeypatch, pg_catalogs)
    monkeypatch.delenv("VELLUM_VAULT_ATTACHMENTS_PATH", raising=False)
...
def test_prefer_hail_overlay(tmp_path, monkeypatch, pg_catalogs):
    vault = tmp_path / "vault"
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(vault))
    rows = []
    ...
    seed_catalog(pg_catalogs, "derived_outputs", {"schema_version": 1, "outputs": rows})
