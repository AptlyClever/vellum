...
def test_register_and_list_element(tmp_path, monkeypatch, pg_catalogs):
...
def test_ingest_manifest_export_models(tmp_path, monkeypatch, pg_catalogs):
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
...
def test_ingest_run_archive(tmp_path, monkeypatch, pg_catalogs):
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
...
def test_upload_run_endpoint(tmp_path, monkeypatch, pg_catalogs):
    import io
    import zipfile
...
def test_publish_to_lane(tmp_path, monkeypatch, pg_catalogs):
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
...
def test_unpublish_from_lane_removes_lane_copy(tmp_path, monkeypatch, pg_catalogs):
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
    src = tmp_path / "clip.webm"
    src.write_bytes(b"webm")
    row = gr.register_element(
...
def test_publish_with_presentation_contract(tmp_path, monkeypatch, pg_catalogs):
    monkeypatch.setenv("VELLUM_VAULT_ROOT", str(tmp_path / "vault"))
    src = tmp_path / "clip.webm"
    src.write_bytes(b"webm")
    row = gr.register_element(
...
def test_publish_presentation_rejects_invalid(tmp_path, monkeypatch, pg_catalogs):
    import pytest
