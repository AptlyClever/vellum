# Perforce (P4) for the VellumLibrary Library

**Product SoT:** [`asset-pipeline-product.md`](./asset-pipeline-product.md)  
**Library project:** `D:\Games\VellumLibrary`  
**Free tier:** Perforce Helix Core — up to 5 users / 20 workspaces self-hosted.

## Why P4 (not git) for Content

Unreal `.uasset` / `.umap` binaries do not merge. P4 file locking + large binary storage is the Epic-recommended pattern. **Git stays** for the Vellum code/tools repo (`D:\dev\vellum`).

## Topology (LAN)

| Piece | Host | Notes |
| --- | --- | --- |
| `p4d` server | Factory (or hub) | Port `1666`, depot `//vellum_library/...` |
| Workspace | Factory | Root `D:\Games\VellumLibrary` |
| Clients | operators / CI | Optional second workspace on hub for read-only sync |

## Bootstrap (Factory admin)
...
```text
1. Fab Add-to-Project → VellumLibrary (lands at Content/<Pack>; leave it there)
2. p4 reconcile //vellum_library/VellumLibrary/Content/...
