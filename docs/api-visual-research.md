  ...
  "items": [
    {
      ...
      "mneme_document_url": "http://127.0.0.1:8790/api/documents/doc-20260718-...",
...
```
http://127.0.0.1:8770/api/visual-research/<id>/file
```
...
```bash
curl -sS -X POST "http://127.0.0.1:8770/api/visual-research/bundles" \
...
```bash
curl -sS -X POST "http://127.0.0.1:8770/api/visual-research" \
  -H "Authorization: Bearer $VELLUM_RESEARCH_WRITE_TOKEN" \
  -F "file=@./capture.png" \
  -F "title=Neon HUD mockup" \
  -F "source_url=https://example.com/page" \
...
- `VELLUM_VAULT_RESEARCH_PATH` (human-readable mirror only -- storage itself is Postgres, `vellum.catalogs` name `"visual_research"`)
- `VELLUM_RESEARCH_WRITE_TOKEN`
