...
| 2 | Human | Fab **Add to Project** → **VellumLibrary** only (`D:\Games\VellumLibrary`). Pack lands at `Content/<PackFolder>` — leave it there. **Never move pack folders on disk** (breaks .uasset references; see [`library-project.md`](./library-project.md)) |
...
- Install Fab packs into any project other than VellumLibrary

## Register fields that matter

- `content_root` — Unreal path (`/Game/<PackFolder>`)
- `host_content_path` — Windows path under Factory Library
...
`tools/pipeline/reconcile_aurora.ps1` runs on Factory at logon + hourly
...
10. Launcher visibility guard — keeps `CreatedProjectPaths=D:/Games` (the
   **parent** folder, not the project folder) in the launcher ini so
   VellumLibrary always appears in Fab's *Add to Project* dropdown
11. Stray-project scan (Fab installed into the wrong project)

Everything it cannot fix lands in
`D:\Games\VellumLibrary\Saved\VellumReconcile\reconcile_report.json`
...
### How "not on Factory" packs are classified
...
| `fab_add_to_project` | Launcher owns it; one click: Fab Library → Add to Project → VellumLibrary | Human, once |
| `fab_add_to_project_unseen` | Launcher on Factory has never seen the pack; find it in Fab Library first | Human, once |
...
Add to Project with "show all projects" if the UE 5.8 VellumLibrary is hidden.
