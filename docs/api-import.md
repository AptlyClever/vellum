...
Operator stays in Vellum. Hub → Factory agent installs from Epic VaultCache and
...
Launcher project picker is unreliable for `D:\Games\VellumLibrary`.
...
2. Vellum **Install from VaultCache** → job `host_fab_install` robocopies into
   `D:\Games\VellumLibrary\Content\` (no Fab UI).
...
| Install into F: Content | Factory agent | `POST …/import/fab-install` → `host_fab_install` |
| Batch install | Operator | `POST /api/import/fab-install-batch` |
| In project | Auto after install / mark | pick scanned folder → mark |
| Open editor | Factory agent | `POST /api/ue/hosts/open-editor` → `host_open_editor` |
| Content folders | Factory scan | `GET /api/ue/hosts/content-folders` · `POST …/refresh` → `host_scan` |
| Stage | Factory agent | `POST …/import/stage` → `host_stage` |
