...
and continuation contract), [`machine-roles.md`](./machine-roles.md) (Studio
dev-primary / Factory factory-primary split), `docs/capture-hosting-decision.md`
...
| Layer | Role |
| --- | --- |
| **Asset Library** | One curated Unreal project (`VellumLibrary`) with one folder per pack at `Content/<Pack>`, versioned in Perforce |
| **Conversion Factory** | Factory reconcile runs three isolated parallel pack workers; each pack pays one headless `UnrealEditor-Cmd` boot for `factory-all` (models + media + VFX plan) |
...
- **Human boundary:** Epic/Fab ownership and Add to Project are human UI steps.
  Everything after content appears in `VellumLibrary/Content` is machine-owned.
...
- **One Library:** Fab installs only into
  `D:\Games\VellumLibrary`; never move `.uasset` folders on disk.
- **Machine split:** Studio is the primary development workstation. Factory
  remains the primary asset/factory workstation and the active UE host; agents
  may still run directly on Factory for asset/factory/host-specific work.
