...
**Status:** implemented (scripted batch) / Factory clean Capture proof (Chrys + pack harden 2026-07-13)  
**Pilot asset:** `fireworks-vol-1-niagara`  
**Primary host:** Factory (`config/ue-hosts.json` → `active: factory`)  
...
2. Active UE **asset/factory host profile** runs the job. Factory remains the
   factory host; Studio is the primary dev workstation unless a deliberate
...
| Host-profile aware (Factory / Studio) | Tied to one machine’s hardcoded `C:\Program Files\…` |
...
| **Host profile** | `config/ue-hosts.json` — UE path, scratch `.uproject`, content root | Shipped (Factory active) |
| **Agent shell** | Poll, resolve host, invoke runner, upload, report | Shipped |
| **Inventory** | List / pick Niagara systems under content root | Exists; needs content on Factory |
...
1. **Factory** scratch project exists: `D:\Games\VellumLibrary\VellumLibrary.uproject` (profile).
...
Windows agent POSTs a CIM snapshot to `POST /api/ue/hosts/specs` on startup (`report_host_specs.ps1`). Surfaced on `GET /api/ue/hosts` as `host_specs` so Capture planning can see real CPU/GPU/RAM instead of treating Factory as a black box.
...
| id | role | notes |
| --- | --- | --- |
| `factory` | asset/factory primary / **active** | `D:\Games\UE_5.8\…`, project `D:\Games\VellumLibrary` |
| `studio` | dev primary / secondary UE profile | Epic Launcher default paths; do not move factory here without migration plan |
...
1. Confirm Fireworks present under Factory scratch (`/Game/FireworksV1`) + MRQ/Python plugins — **done 2026-07-13**.
2. Implement scripted capture: inventory → stage → Sequencer assets → MRQ (cmdline) → heroes + sequence → ingest (**slots** + **hail-overlay**).
3. First proof run via agent/`ue_capture` on Factory (one system OK for first green); then default `max_systems`.
...
| `config/ue-hosts.json` | Factory / Studio paths |
