...
function renderLiveOps(ops) {
  ...
  if (h.gpu_util_pct != null || h.worker_busy != null) {
    ...
    util.textContent = `Factory ${h.host_id || ""}: ${gpu}${mem}${ed}${busy}${tax}`;
...
function renderRows(assets) {
  ...
  for (const a of assets) {
    ...
    tr.addEventListener("click", () => {
      if (typeof window.openVellumJourney === "function") {
        window.openVellumJourney(a.id);
      } else {
        window.location.hash = `#/assets/${encodeURIComponent(a.id)}`;
      }
    });
    body.appendChild(tr);
  }
...
async function openDetail(id) {
  ...
  journeyLink.className = "btn";
  journeyLink.href = `#/assets/${encodeURIComponent(a.id)}`;
  journeyLink.textContent = "Open Asset Journey";
  journeyLink.addEventListener("click", () => {
    $("detail").hidden = true;
  });
  host.appendChild(journeyLink);
  ...
  const fabLabel =
    ...
    "VellumLibrary (F:\\Games\\VellumLibrary)";
  const fabProject =
    (activeHost &&
      (activeHost.fab_target_project ||
        (activeHost.host_specs && activeHost.host_specs.fab_target_project) ||
        activeHost.project)) ||
    "F:\\Games\\VellumLibrary\\VellumLibrary.uproject";
  fabBanner.textContent =
    "STOP using Epic Launcher Fab Add-to-Project (project picker is broken). Open VellumLibrary in Unreal 5.8, then use the Fab plugin INSIDE the editor to Add to this project. Packs already on F: — pick below and Stage.";
  ...
  openEditorBtn.textContent = "Open VellumLibrary in UE";
  openEditorBtn.addEventListener("click", async () => {
    openEditorBtn.disabled = true;
    openEditorBtn.textContent = "Launching…";
    try {
      const hostId = (activeHost && activeHost.id) || "factory";
      const res = await fetch(
  ...
  pickLabel.textContent = "Content folder (from Factory scan)";
  ...
  fabInstallBtn.title =
    "Copy Epic VaultCache pack into F:\\Games\\VellumLibrary\\Content (no Fab UI)";
    ...
    try {
      const res = await fetch(
        `/api/assets/${encodeURIComponent(a.id)}/import/fab-install`,
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ue_host: (activeHost && activeHost.id) || "factory",
            auto_stage: true,
          }),
        }
      );
  ...
  // The polling Capture prototype is frozen. Retain compatibility code for
  // existing active jobs, but never present it as operator work.
  captureSec.hidden = true;
  captureSec.setAttribute("aria-hidden", "true");
  ...
  if (activeHost) {
    ...
    hostLine.textContent = "UE host unknown — start vellum_ue_agent on Factory";
  }
...
(function applyAxiomEffective() {
  if (document.body.dataset.embed === "axiom") return;
  fetch("/api/axiom-effective")
    ...
    .then((data) => {
      ...
      try {
        ...
        if (branding.document_title) {
          document.title = branding.document_title;
        }
        ...
        const tokens =
          data.resolved_token_overrides || (data.theme && data.theme.tokens) || {};
        ...
        /* ignore malformed payload, keep Vellum's own CSS defaults */
      }
    })
    .catch(() => {
      /* Axiom unreachable — keep Vellum's own CSS defaults */
    });
})();
...
try {
  ...
  if (["register", "studio", "eidolon", "research"].includes(requestedView)) {
    setView(requestedView);
  }
} catch {
  /* keep register as the local fallback */
}
...
async function refreshStudioDesk(lane) {
  ...
  if (laneIndicator) {
    const priorityLabel = currentStudioLane === "bandit" ? "Bandit (1st Priority)" : (currentStudioLane === "project-a" ? "Project A (2nd Priority)" : "Field Ops (Pipeline Ready)");
...
function syncLegacySurface() {
  if ($("legacy-app")?.hidden) {
    if (liveOpsTimer != null) clearInterval(liveOpsTimer);
    liveOpsTimer = null;
    return;
  }
...
syncLegacySurface();
window.addEventListener("hashchange", syncLegacySurface);
