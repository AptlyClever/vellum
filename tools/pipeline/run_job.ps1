...
<#
.SYNOPSIS
  Run a Conversion Factory job against VellumLibrary via UnrealEditor-Cmd.
#>
param(
...
$hostProf = Get-UeHostProfile -RepoRoot $RepoRoot -HostName "factory"
if (-not $UeCmd) { $UeCmd = $hostProf.ue_cmd }
if (-not $Project) { $Project = $hostProf.project }
if (-not $ContentRoot) { $ContentRoot = "/Game/$Pack" }
if (-not $WorkDir) {
  $WorkDir = "D:\Games\VellumLibrary\Saved\VellumPipeline"
}
# Outputs stage locally and reach the vault via the hub HTTP API (upload-run).
# Factory never mounts or writes the vault filesystem directly.
...
if ($Job -eq "bake-vfx" -or $Job -eq "factory-all") {
  ...
  if (Test-Path $packScript) {
    $allowPartial = $AllowPartialVfx -or $RunVfxMrq
    & $packScript -Pack $Pack -WorkDir $WorkDir -OutDir (Join-Path $VaultGameReady "vfx\$Pack") `
      -RequireArtifacts:$RunVfxMrq -AllowPartialArtifacts:$allowPartial
