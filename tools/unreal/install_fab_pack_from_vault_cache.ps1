...
<#
.SYNOPSIS
  Install a Fab/Epic owned pack from local VaultCache into VellumLibrary Content.

.DESCRIPTION
  Epic Launcher already downloaded packs under
  C:\ProgramData\Epic\EpicGamesLauncher\VaultCache\<pack>\data\Content\.
  This copies that Content subtree into the F: VellumLibrary project — no Fab UI.

.EXAMPLE
  pwsh -File install_fab_pack_from_vault_cache.ps1 `
    -ProjectContent "D:\Games\VellumLibrary\Content" `
    -ContentRelPaths @("Hangar-X") `
    -OutJson "D:\Games\VellumLibrary\Saved\VellumFabInstall\hangar-x.json"
#>
param(
