...
<#
.SYNOPSIS
  RETIRED warm Lookdev Worker supervisor (frozen).
...
if ($env:VELLUM_ALLOW_RETIRED_LOOKDEV_WORKER -ne "1") {
  Write-Error @"
vellum_ue_worker.ps1 is RETIRED (Lookdev Worker frozen).
Product path: docs/asset-pipeline-product.md + tools/pipeline/
To run anyway: `$env:VELLUM_ALLOW_RETIRED_LOOKDEV_WORKER = '1'
"@
  exit 2
}
