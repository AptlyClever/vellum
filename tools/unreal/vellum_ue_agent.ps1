...
# BINDING 2026-07-14 (docs/capture-hosting-decision.md):
# Primary = Epic batch MRQ via run_vellum_capture.ps1 (UnrealEditor-Cmd).
# Warm Lookdev Worker is frozen — only when -UseLookdevWorker is explicit.
if ($LegacyCmdRunner) {
  $UseLookdevWorker = $false
}
# Do not force Lookdev Worker on. Default remains Cmd unless switch present.
