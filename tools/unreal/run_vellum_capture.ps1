...
param(
  ...
  [string]$VellumBase = "http://127.0.0.1:8770",
  [string]$Lane = "slots",
...
function Invoke-UeLogged {
  ...
  # and ExitCode on redirected handles hang PowerShell forever (observed Factory).
  ...
  while ($true) {
    ...
    if (-not $alive) {
      # One retry — brief spawn races / process table lag on Factory.
...
function Invoke-ExeQuiet {
  ...
  # time (wrong). NEVER `$null = & exe` / PS Process.ExitCode (Factory hangs).
  param(
