...
from . import axiom_effective as axiom_effective_mod
...
@app.get("/api/kanon-capabilities")
def kanon_capabilities() -> dict[str, Any]:
    """Report what Kanon page compositions may bind to in this app.

    Kanon observes this endpoint (`POST /api/design/v1/capabilities/{app_id}/refresh`)
    and pools the result into Page Studio's component palette. Only declare ids this app
    actually serves — an id here that the runtime cannot resolve produces a composition
    that validates and then renders nothing.
    """
    return {
        "schema_version": 1,
        "app_id": "vellum",
        "runtime": "vanilla",
        "routes": ["#/assets/:asset_id"],
        "data_sources": [
            "vellum.asset.journey",
            "vellum.assets.list",
            "vellum.game-ready.elements",
            "vellum.import.queue",
            "vellum.intake.runs",
            "vellum.jobs.list",
            "vellum.lookdev.outputs",
            "vellum.ops.now",
            "vellum.register.summary",
        ],
        "links": ["vellum.assets.detail"],
        "actions": [],
        "slots": [
...
async def api_import_fab_listings_db(
    ...
    """Factory pushes the launcher's Fab library catalog (listings_v1.db).
...
@app.get("/api/assets/{asset_id}/journey")
def api_asset_journey(asset_id: str) -> dict[str, Any]:
    try:
        return journey_mod.build_asset_journey(asset_id)
    except KeyError:
...
def api_asset_import_stage(asset_id: str, body: ImportStageRequest) -> dict[str, Any]:
    ...
    try:
        ...
        host_id = body.ue_host or "factory"
...
def api_asset_fab_install(asset_id: str, body: ImportFabInstallRequest) -> dict[str, Any]:
    """Enqueue host_fab_install: copy Epic VaultCache pack into VellumLibrary Content."""
    if register_mod.get_asset(asset_id) is None:
        raise HTTPException(status_code=404, detail="asset_not_found")
    try:
...
def api_ue_content_folders_refresh(host_id: str | None = Query(default=None)) -> dict[str, Any]:
    """Enqueue host_scan so Factory re-runs report_host_specs (includes Content/*)."""
    try:
...
def api_ue_open_editor(host_id: str | None = Query(default=None)) -> dict[str, Any]:
    """Enqueue host_open_editor — open canonical VellumLibrary in UE for Fab-in-Editor."""
    try:
        host = ue_hosts_mod.get_host(host_id)
        hid = host.get("id")
    except KeyError as exc:
...
def api_ue_hosts() -> dict[str, Any]:
    """Factory/Studio UE host profiles (active = asset/factory host)."""
    try:
...
async def api_game_ready_upload_run(
    ...
    """Factory uploads a zip of a Conversion Factory run's output tree for a pack."""
    if register_mod.get_asset(asset_id) is None:
        raise HTTPException(status_code=404, detail="asset_not_found")
...
@app.get("/api/axiom-effective")
def api_axiom_effective() -> dict[str, Any]:
    """Axiom's resolved branding/theme for this app; {} if Axiom is unreachable."""
    return axiom_effective_mod.fetch_axiom_effective() or {}
...
def api_studio_briefs(
    ...
    return {
        "schema_version": 1,
        "lane": lane,
        "priority": "1st (Bandit)" if lane == "bandit" else ("2nd (Project A)" if lane == "project-a" else "ready"),
