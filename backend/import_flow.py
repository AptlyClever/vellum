...
appears in VellumLibrary, reconcile owns register, stage, P4, validation, and
...
def _lookdev_asset_ids() -> set[str]:
    ...
    # element counts — the reconcile loop on Factory runs the factory jobs and
...
def availability_row(
    *,
    ...
    return {
        "state": "need_download",
        "label": "Not on Factory",
        "detail": "not under F:\\Games\\VellumLibrary\\Content (parked)",
    }
...
def availability_index(
    *,
    ...
    global _AV_INDEX_CACHE, _AV_INDEX_CACHE_AT, _AV_INDEX_CACHE_KEY
    reg_version = register_mod.catalog_version()
    cache_key = f"{engine or ''}|{host_id or ''}|{reg_version}"
    now = time.monotonic()
    if (
        not force_refresh
...
def ops_pulse(*, engine: str = "unreal", host_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "operator": {
            "responsibility": "none",
            "redeem": "closed",
            "how_to_watch": "http://127.0.0.1:8770/ — Live ops strip",
        },
        "finish": {
            "done": finish_done,
...
def _host_utilization(host_id: str | None = None) -> dict[str, Any]:
    """Last agent-reported Factory util (nvidia-smi + editor) — cheap for pulse."""
    try:
        host = ue_hosts_mod.get_host(host_id)
        hid = str(host.get("id") or "factory")
    except Exception:  # noqa: BLE001
        hid = (host_id or "factory").strip().lower() or "factory"
    ...
    return {
        **pulse,
        ...
        "operator": {
            "responsibility": "none",
            "redeem": "closed",
            "how_to_watch": "http://127.0.0.1:8770/ — Live ops strip (poll /api/ops/pulse)",
            "do_not": [
...
def import_status(asset_id: str) -> dict[str, Any]:
    ...
    steps = [
        ...
        {
            "id": "in_project",
            "label": "Add to Project (Fab → VellumLibrary)",
            "done": in_project,
            "actor": "operator",
        },
        {
    ...
    if mode == "niagara_mrq":
        ...
        job = jobs_mod.enqueue_job(
            ...
            payload={
                ...
                "lane": "slots",
                "project_path": host.get("project_dir") or r"D:\Games\VellumLibrary",
                "content_root": st["content_root"],
                "engine_version": host.get("engine_version") or "5.8",
                "ue_host": host.get("id") or "factory",
                "force": False,
                "max_systems": 0,
                "lookdev_mode": mode,
            },
...
# Content folder name on Factory → register asset id (Humble + known frees).
...
def enqueue_fab_install(
    ...
    *,
    ...
    """Enqueue Factory host_fab_install (VaultCache → project Content)."""
    from . import jobs as jobs_mod
    ...
    try:
        ...
        host_id = ue_host or "factory"
        project_content = r"D:\Games\VellumLibrary\Content"
    job = jobs_mod.enqueue_job(
