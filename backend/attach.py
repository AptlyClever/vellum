...
The attachment log itself is Postgres (``vellum.catalogs``, name
``"attachments"``), not a YAML file -- the same unlocked read-whole-file/mutate/
write-whole-file round trip ``backend/register.py``'s asset catalog had before
PX001 Wave 4, applied to every attach here. See ``backend/pg_catalog.py``.
"""
...
def load_attachments() -> dict[str, Any]:
    return pg_catalog.load_catalog(CATALOG_NAME, _empty())


def _mirror_attachments(doc: dict[str, Any]) -> None:
    """Best-effort human-readable export into the vault index.

    Diagnostic only, same as register.py's _mirror_to_vault: Postgres is the
    source of truth, so a failure here must never break a request that already
    committed.
    """
...
def axiom_base_url() -> str:
    return (os.environ.get("AXIOM_BASE_URL") or "http://127.0.0.1:7895").rstrip("/")
...
def hail_glyph_images_dir() -> Path:
    ...
    if configured:
        return Path(configured)
    return Path("/srv/axiom/config/hails/glyph-hero-images")
...
def lcard_media_dir() -> Path:
    ...
    if configured:
        return Path(configured)
    return Path("/srv/lcard/app/media/vellum")
...
def bandit_static_dir() -> Path:
    ...
    if configured:
        return Path(configured)
    return Path("/srv/bandit/web/vellum")


def bandit_base_url() -> str:
    return (os.environ.get("BANDIT_BASE_URL") or "http://127.0.0.1:8766").rstrip("/")


def lcard_base_url() -> str:
    return (os.environ.get("LCARD_BASE_URL") or "http://127.0.0.1:8184").rstrip("/")
...
def _record_attachment(row: dict[str, Any]) -> dict[str, Any]:
    with pg_catalog.catalog_transaction(CATALOG_NAME, _empty()) as doc:
        rows = list(doc.get("attachments") or [])
        rows.append(row)
        doc["attachments"] = rows
    _mirror_attachments(doc)
    return row
