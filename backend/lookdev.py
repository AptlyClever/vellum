...
The DerivedOutput catalog itself is Postgres (``vellum.catalogs``, name
``"derived_outputs"``), not a YAML file -- see ``backend/pg_catalog.py``.
"""
...
def load_catalog() -> dict[str, Any]:
    return pg_catalog.load_catalog(CATALOG_NAME, _empty_catalog())


def _mirror_catalog(doc: dict[str, Any]) -> None:
    """Best-effort human-readable export into the vault index (see register.py)."""
...
def derive_stills_for_asset(
    ...
    *,
    ...
    1) Fab library catalog artwork (the authoritative marketplace image)
    2) Loose staged png/jpg when no Fab match exists
    3) Generated placeholder PNG
    """
    ...
    if url:
        stills = [
            _download_fab_thumbnail(
                url,
                thumb_dir / f"{asset_id}-{secrets.token_hex(3)}.img",
            )
        ]
        source_note = f"Fab catalog thumbnail ({url}) — ingested as durable marketplace evidence."
    else:
        stills = _candidate_stills(stage, limit=max_stills)
        source_note = (
            "Reference still copied from staged pack textures "
            "(not marketplace artwork or a Niagara viewport render)."
        )
        if not stills:
            stills = [
    ...
    if url:
        # The catalog artwork is already durable under 06-readouts/_fab-thumbs.
        # Register those bytes directly: copying them into historical lane folders
        # is redundant and can collide with factory-owned directory permissions.
        source_file = stills[0]
        created: list[dict[str, Any]] = []
        with pg_catalog.catalog_transaction(CATALOG_NAME, _empty_catalog()) as catalog:
    ...
    for lane in target_lanes:
        ...
        for idx, src in enumerate(stills):
            # Marketplace artwork may already exist from an older derive under a
            # different container uid. Keep ingestion append-only and provenance-safe.
            dest_name = (
                f"{stamp}-{secrets.token_hex(2)}-{src.name}"
                if "Fab catalog" in source_note
                else src.name
            )
