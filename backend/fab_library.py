...
at ``VaultCache/FabLibrary/listings_v1.db``. Factory's reconcile loop pushes a
...
* Presence in ``local_listing`` — whether the launcher on Factory has ever
...
# Fab's launcher cache currently tells us whether Factory has seen a listing, but
...
def acquisition_for_asset(
    ...
    if installable:
        ...
        hint = (
            ...
            "Content into VellumLibrary. Reconcile will stage, validate, and submit after migration."
        )
    elif listing is not None:
        method = METHOD_FAB_ADD_TO_PROJECT
        hint = (
            "Epic Launcher -> Fab Library -> "
            f"\"{listing['title']}\" -> Add to Project -> VellumLibrary. "
        )
        if ue_only:
        ...
        if override.get("supported_unreal_versions"):
            hint += (
                " If VellumLibrary is hidden because it is UE 5.8, enable Show all "
                f"projects and choose closest supported version {override['supported_unreal_versions']}."
            )
    elif engine == "unreal":
        method = METHOD_FAB_ADD_UNSEEN
        hint = (
            "Launcher on Factory has never seen this pack (not in the Fab "
            f"library cache). Epic Launcher -> Fab Library -> search \"{display}\" "
            "-> Add to Project -> VellumLibrary. UE-only: it cannot be downloaded "
