...
Manifest-driven index under vault 05-derived-renders/game-ready/. Catalog itself is
Postgres (``vellum.catalogs``, name ``"game_ready"``), not a YAML file -- see
``backend/pg_catalog.py``.
...
def ingest_run_archive(extract_dir: Path, *, asset_id: str, pack: str) -> dict[str, Any]:
    ...
    Factory zips its local game-ready output tree for a pack and uploads it;
